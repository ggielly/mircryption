...
char *decrypt_string_new_alt(char *key, char *str)
{
	char *p, *s, *dest;
	char *dest2,*dest3;
	int i;
	...
	s = new char[strlen(str) + 12+400];
	strcpy(s, str);

	if ((!key) || (!key[0]))
		return s;
 	dest = new char[strlen(str) + 12 + 8+400];

	std::string decodedstr=Base64::decode(std::string(s));
	ilen=decodedstr.length();
 	dest3 = new char[strlen(str) + 12 + 8+400];
	memcpy(dest3,decodedstr.c_str(),ilen);
	dest3[ilen]=0;

	if (ilen==0 || ilen >strlen(str)+12)
		{
		strcpy(dest3,"FUCKED");
		delete s;
		return dest3;
	...
	CBlowFish oBlowFish((unsigned char*)key, strlen(key));
	oBlowFish.ResetChain();
	oBlowFish.Decrypt((unsigned char*)dest3,(unsigned char*)dest, ilen, CBlowFish::CBC);
	dest[ilen]=0;

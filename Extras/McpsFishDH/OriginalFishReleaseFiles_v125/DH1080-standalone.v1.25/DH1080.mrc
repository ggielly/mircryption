...
; script. It will NOT work with very old
; blowcrypt versions, which store the keys
; plain-text in the blow.ini, use (DH1080-P.mrc)
; instead!
;
...
alias FiSH.setkey {
  ...
  var %ky = $dll(FiSH.DLL, encrypt_key, $2-)
...
alias FiSH.showkey {
  ...
  if ($len(%ky) > 4) {
    %ky = $dll(FiSH.DLL,decrypt_key, %ky)

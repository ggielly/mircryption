...
; like this (adjust blow.ini path manually):
; ===================
; [keys]
; nick1=theKey
; #chan2=SomeChanKey
...
on 1:OPEN:?:{
  if (%autokeyx == [On]) {
    var %tmp1 = $readini addons\blow.ini keys $nick
    if (($len(%tmp1) > 0) || (+OK isin $1)) {
      FiSH.DH1024_INIT $nick
    }
  }
}
...
alias FiSH.setkey {
  ...
  var %ky = $2-
  writeini -n addons\blow.ini keys %cur_contact %ky
  unset %ky
...
alias FiSH.showkey {
  ...
  flushini mirc.ini
  var %ky = $readini addons\blow.ini keys %cur_contact
  if ($len(%ky) > 4) {
...
alias FiSH.removekey {
  ...
  remini addons\blow.ini keys %cur_contact
  flushini mirc.ini

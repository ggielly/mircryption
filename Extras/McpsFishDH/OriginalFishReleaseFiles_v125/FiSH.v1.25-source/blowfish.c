...
// mIRC syntax: var %key = $dll(FiSH.DLL, decrypt_key, %encrypted_key)
mIRC_export decrypt_key(HWND mWnd, HWND aWnd, char *theData, char *params, BOOL show, BOOL nopause)
{
	int i;
	...
	if(strncmp(theData, "+OK ", 4) != 0) return 3;	// key is stored plain-text
...
// mIRC syntax: var %encrypted_key = $dll(FiSH.DLL, encrypt_key, %key)
mIRC_export encrypt_key(HWND mWnd, HWND aWnd, char *theData, char *params, BOOL show, BOOL nopause)
{
	int i;
	unsigned char theContactKey[500];
...
// mIRC syntax: var %crypted_msg = $dll(FiSH.DLL, FiSH_encrypt_msg, %key %msg)
mIRC_export FiSH_encrypt_msg(HWND mWnd, HWND aWnd, char *theData, char *params, BOOL show, BOOL nopause)
{
	int i=0;
	unsigned char bf_dest[2000], theContactKey[500], *msgPtr;

	while(theData[i]!=' ' && theData[i]!=0 ) i++;
	if((i >= 500) || (i < 2) || (theData[i+1]==0) || (theData[i]==0))
	{	// bad arguments
		ZeroMemory(theData, i);
		return 0;	// return 0 halts mIRC
	}

	strncpy(theContactKey, theData, i);
	theContactKey[i]=0;

	msgPtr=theData+i+1;

	encrypt_string(theContactKey, msgPtr, bf_dest, strlen(msgPtr));
	ZeroMemory(theContactKey, sizeof(theContactKey));

	bf_dest[512]=0;
	strcpy(theData, "+OK ");
	strcat(theData, bf_dest);
	ZeroMemory(bf_dest, sizeof(bf_dest));

	return 3;
}

// mIRC syntax: var %decrypted_msg = $dll(FiSH.DLL, FiSH_decrypt_msg, %key %encrypted_msg)
mIRC_export FiSH_decrypt_msg(HWND mWnd, HWND aWnd, char *theData, char *params, BOOL show, BOOL nopause)
{
	int i=0;
	unsigned char bf_dest[2000], theContactKey[500], *msgPtr;
	...
	if(strncmp(msgPtr, "+OK ", 4)==0) msgPtr += 4;		// strip "+OK "

	decrypt_string(theContactKey, msgPtr, bf_dest, strlen(msgPtr));
	ZeroMemory(theContactKey, sizeof(theContactKey));

	bf_dest[512]=0;
	strcpy(theData, bf_dest);
	ZeroMemory(bf_dest, sizeof(bf_dest));

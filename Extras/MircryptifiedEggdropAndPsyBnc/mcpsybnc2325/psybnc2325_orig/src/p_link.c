...
#ifndef lint
static char rcsid[] = "@(#)$Id: p_link.c,v 1.1.1.1 2005/02/06 12:56:17 hisi Exp $";
#endif
...
int linkrelayconnected(int nlink)
{
    ...
    log(LOG_INFO,-1,lngtxt(509),datalink(nlink)->iam);
    return 0x0;
}
...
int errorrelaylink(int nlink, int errn)
{
    ...
    pcontext;
    log(LOG_ERROR,-1,lngtxt(510),datalink(nlink)->iam);
...
int killrelaylink(int nlink)
{
    struct socketnodes *lkm;
    pcontext;
    log(LOG_WARNING,-1,lngtxt(511),datalink(nlink)->iam);
    lkm=getpsocketbysock(datalink(nlink)->outsock);
    if(lkm!=NULL)
    {
...
int checklinkkill(int nlink,int errn)
{
    ...
    log(LOG_WARNING,-1,lngtxt(557),nlink);
    killsocket(sock);
    datalink(nlink)->instate=STD_NOCON;
    datalink(nlink)->outstate=STD_NOCON;
    datalink(nlink)->insock=0;
...
int linkrelay(int npeer, int rootlink)
{
    ...
    log(LOG_ERROR,-1,lngtxt(560),newpeer(npeer)->server,newpeer(npeer)->port);
...
int connectlink(int nlink)
{
    ...
    log(LOG_INFO,-1,lngtxt(561),vsl,nlink,ho,datalink(nlink)->port);
    ...
    if(tmpsock!=0)
	return 0x1;
    log(LOG_ERROR,-1,lngtxt(562),nlink,datalink(nlink)->host,datalink(nlink)->port);
    datalink(nlink)->outstate=STD_NOCON;
    return -1;
}

 ...
 *
 * $Id: p_coredns.c,v 1.1.1.1 2005/02/06 12:56:19 hisi Exp $
 */
/*
...
{
    killsocket(resfd);
    log(LOG_INFO,-1,lngtxt(1351));
    if(init_dns_network()==0)
    {
	log(LOG_ERROR,-1,lngtxt(1352));
	exit(0x1);
    }
}
...
static int init_dns_network(void)
{
    ...
    if (resfd == -1) {
	log(LOG_ERROR, -1,lngtxt(1353),strerror(errno));
	return 0;
    }
    option = 1;
    if (setsockopt(resfd, SOL_SOCKET, SO_BROADCAST, (char *) &option,
	 sizeof(option))) {
	log(LOG_ERROR, -1,lngtxt(1354),strerror(errno));
	killsocket(resfd);
	return 0;
    }
...
void dns_stat(int usern)
{
    ...
    log(LOG_INFO,usern,"Logging DNS-Stats");
    log(LOG_INFO,usern,"%-8lu open DNS-Queries",openq);
    log(LOG_INFO,usern,"%-8lu finished DNS-Queries",finishedq);
    log(LOG_INFO,usern,"%-8lu failed DNS-Queries",failedq);
    log(LOG_INFO,usern,"%-8lu cached Answers",cachedq);
    log(LOG_INFO,usern,"End of DNS-Stats");
    return;
}

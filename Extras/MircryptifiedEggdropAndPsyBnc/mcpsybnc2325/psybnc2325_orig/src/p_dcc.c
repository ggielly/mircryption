...
#ifndef lint
static char rcsid[] = "@(#)$Id: p_dcc.c,v 1.1.1.1 2005/02/06 12:56:16 hisi Exp $";
#endif
...
int adddcc(int usern, char *host, int port, char *luser, char *pass, char *name,int noini)
{
    ...
    log(LOG_INFO,usern,lngtxt(289),thisdcc->link->name,thisdcc->link->host,thisdcc->link->port,user(usern)->login);
    return 0x0;
}
...
int connecteddcc(int usern)
{
    ...
    if (lkm==NULL) return 0x0;
    th=lkm->link;
    log(LOG_INFO,usern,lngtxt(304),user(usern)->login,th->name,th->host,th->port);
    th->delayed=1;
...
int killeddcc(int usern)
{
    ...
    if (lkm==NULL) return 0x0;
    th=lkm->link;
    log(LOG_WARNING,usern,lngtxt(307),user(usern)->login,th->name,th->host,th->port);
    th->delayed=1;
...
int errordcc(int usern, int errn)
{
    ...
    th=lkm->link;
    log(LOG_ERROR,usern,lngtxt(308),user(usern)->login,th->name,th->host,th->port);
    th->delayed=1;
    th->outstate=0;
    killsocket(th->outsock);
    return 0x0;	
...
int checkdcclink(int usern, struct datalinkt *th)
{
	...
	if(issl==SSL_ON)
    	    log(LOG_INFO,usern,lngtxt(309),user(usern)->login,th->name,ho,th->port);
	else
#endif
    	    log(LOG_INFO,usern,lngtxt(310),user(usern)->login,th->name,ho,th->port);
	...
	if (th->outsock==0)
	{
    	    log(LOG_ERROR,usern,lngtxt(311),user(usern)->login,th->name,ho,th->port);
	    th->delayed=1;
	    return 0x0;	
	}
	return 0x1;
...
int pdccfilesendack(int usern)
{
	...
	if(user(usern)->instate==STD_CONN)
	{
	    log(LOG_INFO,usern,lngtxt(321),pdcc->file,netc,pdcc->nick);
	}
	fclose(pdcc->fhandle);
	removepdcc(usern);
    } else {
...
int pdccfconnected(int usern)
{
    ...
    if(pdcc->fhandle==NULL)
    {
	log(LOG_ERROR,usern,lngtxt(325),pdcc->file);
...
int pdccfclosed(int usern)
{
	...
	if(pdcc->filesize!=pdcc->transferred)
	    ...
	    log(LOG_INFO,usern,lngtxt(330),pdcc->file,netc,pdcc->nick,pdcc->host);
    }    
    if(pdcc->fhandle!=NULL)
	fclose(pdcc->fhandle);    
    removepdcc(usern);
...
int addpendingdcc(int usern, int type, char *host, int port, char *nick, char *file, char *newfile, unsigned long fsize, int eproto)
{
	...
	case PDC_CHATTORQ:
	    ...
	    if(dcchost[0]==0)
	    {
		log(LOG_ERROR,usern,lngtxt(332));
		return -1;
	    }
	    sck=0;cnt=0;
	    while(sck<=0 && cnt<20)
	    ...
	    if(sck<=0)
	    {
		log(LOG_ERROR,usern,lngtxt(333));
		return -1;
	    }
	    ps=getpsocketbysock(sck);
	    if(ps==NULL)
	    {
		log(LOG_ERROR,usern,lngtxt(334));
		return -1;
	    }
	    ps->sock->param=usern;
	...
	case PDC_SENDTORQ:
	    ...
	    if(dcchost[0]==0)
	    {
		log(LOG_ERROR,usern,lngtxt(344));
		return -1;
	    }
	    fil=stripdccfile(file,0);
	    if(fil==NULL)
	    {
		log(LOG_ERROR,usern,lngtxt(345),file);
		return -1;
	    }
	    check=fopen(file,"r");
	    if(check==NULL)
	    ...
	    if(sck<=0)
	    {
		log(LOG_ERROR,usern,lngtxt(348));
		return -1;
	    }
	    ps=getpsocketbysock(sck);
	    if(ps==NULL)
	    {
		log(LOG_ERROR,usern,lngtxt(349));
		return -1;
	    }
	    ps->sock->param=usern;
	    strmncpy(ps->sock->dest,"DCC",sizeof(ps->sock->dest));

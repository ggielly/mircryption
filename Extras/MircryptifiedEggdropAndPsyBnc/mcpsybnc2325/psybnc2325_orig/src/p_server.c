...
#ifndef lint
static char rcsid[] = "@(#)$Id: p_server.c,v 1.1.1.1 2005/02/06 12:56:17 hisi Exp $";
#endif
...
int msgERROR(int usern)
{
    ...
    pcontext;
    log(LOG_ERROR,usern,lngtxt(760),user(usern)->login,user(usern)->network,user(usern)->server,irccontent);
...
int userconnected(int nuser)
{
    ...
    log(LOG_INFO,nuser,lngtxt(778),user(nuser)->login,user(nuser)->network,user(nuser)->server,user(nuser)->port,user(nuser)->vhost);
...
int usererror(int nuser,int errn)
{
	...
	log(LOG_ERROR,nuser,lngtxt(779),user(nuser)->login,user(nuser)->server,user(nuser)->port);
...
int userclosed(int nuser)
{
    ...
    char buf[400];
    pcontext;
    log(LOG_WARNING,nuser,lngtxt(781),user(nuser)->login);
    th=getpsocketbysock(user(nuser)->outsock);
    if(th!=NULL)
    {
	th->sock->destructor=NULL;
...
int connectuser(int nuser)
{
   ...
   if (user(nuser)->outstate == STD_NOCON)
   {
	...
	if (getnextserver(nuser) != 0) {
	    user(nuser)->outstate = STD_NOCON;
	    log(LOG_INFO,nuser,lngtxt(783),user(nuser)->login,user(nuser)->network);
	    memset(user(nuser)->server,0x0,sizeof(user(nuser)->server));
	...
	if (user(nuser)->port == 0) {
	    user(nuser)->outstate = STD_NOCON;
	    log(LOG_INFO,nuser,lngtxt(784),user(nuser)->login,user(nuser)->network);
	    memset(user(nuser)->server,0x0,sizeof(user(nuser)->server));
	    user(nuser)->delayed = 5;
	    return 0x0;
	}
	...
	if (user(nuser)->pport!=0)
	{
	   log(LOG_INFO,nuser,lngtxt(786),user(nuser)->login,user(nuser)->network,user(nuser)->proxy,user(nuser)->pport,user(nuser)->server,user(nuser)->port);
	   user(nuser)->outsock=connectto(user(nuser)->outsock,user(nuser)->proxy,user(nuser)->pport,NULL);
	} else
	if (nlink!=0)
	{
  	   log(LOG_INFO,nuser,lngtxt(787),user(nuser)->login,user(nuser)->network,user(nuser)->server,user(nuser)->port,datalink(nlink)->name,ho,datalink(nlink)->port,user(nuser)->vhost);
	   user(nuser)->outsock=connectto(user(nuser)->outsock,ho,datalink(nlink)->port,NULL);
	} else {
  	   log(LOG_INFO,nuser,lngtxt(788),user(nuser)->login,user(nuser)->network,ho,user(nuser)->port,user(nuser)->vhost);
	...
	{
	    if(*user(nuser)->server)
		log(LOG_ERROR,nuser,lngtxt(789),user(nuser)->login,user(nuser)->server,user(nuser)->port);
	    return 0x0;
	}
...
int checkstonednick()
{
        ...
        if(user(usern)->outstate==STD_CONN && user(usern)->welcome==1)
	{
		...
		if(user(usern)->pingtime+STONETIME<=time(NULL))
		{
		    ...
		    log(LOG_INFO,usern,lngtxt(1357),user(usern)->login,user(usern)->nick);

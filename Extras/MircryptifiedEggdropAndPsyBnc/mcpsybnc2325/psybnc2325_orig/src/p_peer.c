...
#ifndef lint
static char rcsid[] = "@(#)$Id: p_peer.c,v 1.1.1.1 2005/02/06 12:56:17 hisi Exp $";
#endif
...
int linklink(int npeer)
{
	...
	log(LOG_INFO,-1,lngtxt(671),datalink(lnk)->name);
    ...
    */
	...
	if(lkm!=NULL)
	{
		...
		if(pt!=NULL)
		{
		    ...
		    log(LOG_INFO,-1,lngtxt(674),user(rc)->login);
		    writeuser(rc);
		    flushconfig();
		}
	    }
...
#endif
    log(LOG_INFO,-1,lngtxt(677),newpeer(npeer)->login);
    lkm=getpsocketbysock(newpeer(npeer)->insock);
    pcontext;
    if(lkm!=NULL)
    {
...
int killoldlistener(int npeer)
{
    ...
    log(LOG_WARNING,-1,lngtxt(685),newpeer(npeer)->host,newpeer(npeer)->login);
    erasepeer(npeer);
    return;	  
}
...
int erroroldlistener(int npeer,int errn)
{
    pcontext;
    if(npeer==0) return 0x0;
    log(LOG_ERROR,-1,lngtxt(686),newpeer(npeer)->host,newpeer(npeer)->login);
    ...
    if (newpeer(npeer)->state != STD_NOUSE) {
	  ...
	  if (ifcommand("PASS")) {
	     ...
	     if (newpeer(npeer)->type!=NP_USER)
	     {
	         if (linklink(npeer)==-1)
		 {
		     log(LOG_ERROR,-1,lngtxt(693),newpeer(npeer)->host,newpeer(npeer)->name);
	             erasepeer(npeer);
		     pcontext;
		     return 0x0;		     
		 }
	  ...
	  if (strlen(newpeer(npeer)->nick) != 0) {
	      ...
	      if (strlen(newpeer(npeer)->pass) != 0) {
		     ...
		     log(LOG_ERROR,-1,lngtxt(695),newpeer(npeer)->login,newpeer(npeer)->host);
	             erasepeer(npeer);
		  }
	      }
	    }  
...
int checknewlistener(int i)
{
    ...
    npeer=getnewpeer();
    if (npeer == -1) {
	log(LOG_ERROR,-1,lngtxt(699),accepthost);

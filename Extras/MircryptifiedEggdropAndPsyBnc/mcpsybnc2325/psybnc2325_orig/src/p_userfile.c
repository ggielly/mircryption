...
#ifndef lint
static char rcsid[] = "@(#)$Id: p_userfile.c,v 1.1.1.1 2005/02/06 12:56:18 hisi Exp $";
#endif
...
//   clearuser(usernum);
   ...
   if (rc != 0) {
      log(LOG_ERROR,-1,lngtxt(910),user(usernum)->login);
...
int loadusers(void)
{
   ...
   pcontext;
   curuser = 1;
   log(LOG_INFO,-1,lngtxt(964));
   while (curuser <= MAX_USER) 
   {
       loaduser(curuser);
       curuser++;
   }
   if (nousers) log(LOG_WARNING,-1,lngtxt(965));
   th=usernode;
   while (th!=NULL) 
   {
       curuser=th->uid;
...
int loadlink(int linkn)
{
    ...
    if (datalink(linkn)->outstate==STD_CONN) {
        log(LOG_WARNING,-1,lngtxt(966),datalink(linkn)->host,datalink(linkn)->port);
	killsocket(datalink(linkn)->outsock);
    }
    if (datalink(linkn)->instate==STD_CONN) {
        log(LOG_WARNING,-1,lngtxt(967),datalink(linkn)->host,datalink(linkn)->port);

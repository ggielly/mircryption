...
#ifndef lint
static char rcsid[] = "@(#)$Id: p_socket.c,v 1.1.1.1 2005/02/06 12:56:18 hisi Exp $";
#endif
...
int createlistener(char *host,int listenport,int proto,int pending, int(*listenhandler)(int), int(*errorhandler)(int,int), int(*datahandler)(int), int(*closehandler)(int))
{
  ...
  {
      if(pending==0)
          log(LOG_ERROR,-1,lngtxt(793),ho,listenport,vsl);
      return 0;
  }
  ...
  if(proto==AF_INET6)
  {
	  ...
	  listen_sa6.sin6_family=AF_INET6;
          ...
          if(inet_pton(AF_INET6,ho,&listen_sa6.sin6_addr)<=0)
          {
              ...
              if(pending==0)
                  log(LOG_ERROR,-1,lngtxt(794),ho,listenport,vsl);
	      return 0x0;
          }                   
          listen_sa6.sin6_family=AF_INET6;
      }
...
/* ipv6 dcc not yet specified
	  ...
	  listen_sa.sin_family=AF_INET;
          ...
          if(inet_aton(ho,&listen_sa.sin_addr)<=0)
          {
              killsocket(listensocket);
              if(pending==0)
                  log(LOG_ERROR,-1,lngtxt(795),host,listenport,vsl);
	      return 0x0;
          }
          listen_sa.sin_family=AF_INET;
      }
  ...
  {	
      killsocket(listensocket);
      if(pending==0)
          log(LOG_ERROR,-1,lngtxt(796),ho,listenport,vsl);
  ...
  {	
      killsocket(listensocket);
      if(pending==0)
          log(LOG_ERROR,-1,lngtxt(797),ho,listenport,vsl);
      return 0; /* cannot create socket */
  }
  if(pending==0)
  {
      printf(lngtxt(798),lkm->sock->source,listenport,vsl);
      log(LOG_INFO,-1,lngtxt(799),lkm->sock->source,listenport,vsl);
  }
  return listensocket;
}
		...
		*pt=0;	    
		...
		if(lkm)
		{
			...
			if(pt)
			{
				...
				*ept=0;
				port=atoi(pt);
				log(LOG_ERROR,-1,"Cannot resolve Host '%s'. Connect to %s:%d cancelled",host,host,port);
				killsocket(sockt);
			    }			    			    
			}
		    }
...
int socket_connect()
{
	...
	if(rc==-1)
	{
	    ...
	    {
    		log(LOG_ERROR,-1,lngtxt(803),currentsocket->sock->syssock,currentsocket->sock->param);
    		ern=-1;
	    }
	    else
    		ern=0;
	...
	{					
	    ...
	    if(rc==-1)
	    {
    		...
    		if(ern!=SSL_ERROR_WANT_READ && ern!=SSL_ERROR_WANT_WRITE && ern!=SSL_ERROR_NONE)
    		{
    		    log(LOG_ERROR,-1,lngtxt(804),currentsocket->sock->syssock,currentsocket->sock->param);
		    ern=-1;
	        }
		else
		    ern=0;
...
int sslcheckcert(int socket, char *cert, char *logitem)
{
	...
	if(ps->sock->sslfd!=NULL) 
	{
		...
		if(SSLSEC==0)
		{
		    log(LOG_INFO,-1,lngtxt(805),logitem);
		    rc=1;
		} else {
		    log(LOG_ERROR,-1,lngtxt(806),logitem);
		    rc=0;
		}
	    } else {
		if(strmcmp(ccert,cert))		
		{
		    log(LOG_INFO,-1,lngtxt(807),logitem);
		    rc=1;
		} else {
		    if(SSLSEC==0)
		    {
			log(LOG_INFO,-1,lngtxt(810),logitem,ccert);
			rc=1;
		    }
		    else
		    {
			log(LOG_INFO,-1,lngtxt(811),logitem,ccert);
			rc=0;
		    }
		}
	    }
...
int acceptresolved(struct resolve *rp)
{
	...
	*pt=0;
    ...
    {
	log(LOG_ERROR,-1,lngtxt(813),accepthost);
#ifdef HAVE_SSL 
    ...
    log(LOG_WARNING,-1,lngtxt(814),accepthost);
	...
	if(currentsocket->sock->ssl==SSL_ON && currentsocket->sock->sslfd!=NULL)
	{
		...
		switch(rc)
		{
			...
			log(LOG_ERROR,-1,lngtxt(815),accepthost,sebuf);
			killsocket(asocket);
			return -1;
		}
	    }
	    log(LOG_INFO,-1,lngtxt(816),accepthost);
	}
    }
#endif
	...
	if(th!=NULL)
	{
		...
		if(FD_ISSET(ssck,&rfds) || FD_ISSET(ssck,&wfds))
		{
			...
			if(optbuf==0)
			{
				...
				if(th->sock->sslfd==0)
				{
				    log(LOG_ERROR,-1,lngtxt(817),th->sock->syssock,th->sock->param);
				    optbuf=-1;
				}
			    }
#endif

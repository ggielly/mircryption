...
#ifndef lint
static char rcsid[] = "@(#)$Id: p_script.c,v 1.1.1.1 2005/02/06 12:56:14 hisi Exp $";
#endif
		...
		int(*destroyed)(int))
{
    ...
    if(forks>MAXFORKS)
    {
	log(LOG_ERROR,usern,lngtxt(714),program,user(usern)->login);
	return -1;
    }
    if(mainlog!=NULL)
    {
...
/*	write(STD_ERR,lngtxt(715),4);  sending us an EOF  */
	...
	if(ss3<=0)
	{
	    ...
	    log(LOG_ERROR,usern,lngtxt(716),program,user(usern)->login);
	    return -1;
	}
	forks++;

...
#ifndef lint
static char rcsid[] = "@(#)$Id: p_memory.c,v 1.1.1.1 2005/02/06 12:56:17 hisi Exp $";
#endif
...
unsigned long *__pmalloc(unsigned long size,char *module,char *function, int line)
{
    ...
    {
	log(LOG_ERROR,-1,lngtxt(602),module,function,line);
	exit(0x0);
    }	
    memset(rc,0x0,size);
#ifdef LOGALLOC
...
int p_debug()
{
    log(LOG_ERROR,-1,lngtxt(603),ctxt,cfunc,cline);
    return 0x0;
}
...
void usr1_error(int r)
{
	...
	if(lkm->sock!=NULL)
	{
		...
		if(fcntl(lkm->sock->syssock, F_GETFD,0) <0) {
		    log(LOG_WARNING,-1,lngtxt(632),lkm->sock->syssock);

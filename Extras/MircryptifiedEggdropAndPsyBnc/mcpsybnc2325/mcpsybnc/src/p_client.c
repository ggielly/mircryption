 ...
 */
//---------------------------------------------------------------------------
// modified 1/14/05 mouser, for mircryption support
//---------------------------------------------------------------------------


#ifndef lint
static char rcsid[] = "@(#)$Id: p_client.c,v 1.1.1.1 2005/02/06 12:56:15 hisi Exp $";
#endif
...
// mouser, 1/15/05, include mircryption code if appropriate
#ifdef COMPILEMIRCRYPTION
  #include <p_mircryptionwrap.h>
#endif
...
int cmdbwho(usern)
{
    ...
    // mouser 1/21/05, i dont think nonadmin should be able to issue a bwho
    #ifdef ONLYADMINSBWHO
    if(user(usern)->rights!=RI_ADMIN)
    	{
           ssnprintf(user(usern)->insock,lngtxt(1303),me,user(usern)->nick);
           return -1;
    	}
    #endif
...
{
    log(LOG_INFO,usern,lngtxt(22));
    usr1_error(31337);
    log(LOG_INFO,usern,lngtxt(23));
}
...
int cmdadduser(int usern)
{
	...
	if (*user(uind)->login == 0) {
	    ...
	    user(uind)->sysmsg = 1;
	    log(LOG_INFO,usern,lngtxt(31),user(uind)->login,user(uind)->user,user(usern)->login);
	    if(usern!=0)
...
int cmddeluser(int usern)
{
    ...
    deluser(uind);
    log(LOG_INFO,usern,lngtxt(37),user(uind)->nick,user(userp)->login);
    loaduser(uind);
    return 0x0;
}
...
int cmdaddnetwork(int usern)
{
	...
	if (strlen(user(uind)->login) == 0) {
	    ...
	    clearuser(uind);
	    loaduser(uind);
	    log(LOG_INFO,usern,lngtxt(42),user(uind)->network,user(usern)->login);
	    return uind;    	
	}
	uind++;
    }
    log(LOG_ERROR,usern,lngtxt(43));
    return -1;
}
...
int cmddelnetwork(int usern)
{
    ...
    deluser(uind);
    log(LOG_INFO,usern,lngtxt(46),user(uind)->network,user(uind)->login);
    loaduser(uind);
}
...
int cmdjump(int usern)
{
	...
	log(LOG_INFO,usern,lngtxt(60),user(usern)->login);
...
int cmdaddlink(int usern,int type)
{
    ...
    *pt=0;pt++;
    ...
    writelink(newlink);
    log(LOG_INFO,usern,lngtxt(135),datalink(newlink)->name,tp,datalink(newlink)->host,datalink(newlink)->port,user(usern)->login);
    return 0x0;
}
...
int cmdrelaylink(int usern)
{
	...
	pt=yo;
    log(LOG_INFO,usern,lngtxt(139),ircto,pt,user(usern)->login);
    return 0x0;
}
...
int cmddellink(int usern)
{
    ...
    log(LOG_INFO,usern,lngtxt(142),lnk,user(usern)->login);
...
int cmdrehash(int usern) {
    ...
    log(LOG_WARNING,usern,lngtxt(246),user(usern)->login);
    sck=socketnode;
...
int cmdadmin(int usern) {
    ...
    if (rc) {
       ...
       if(brem)
       {
	    log(LOG_INFO,usern,lngtxt(248),user(userp)->nick,irccontent);
       }
    } else {
...
int cmdunadmin(int usern) {
    ...
    if (rc) {
       ...
       if(brem)
       {
           log(LOG_INFO,usern,lngtxt(251),user(userp)->nick,irccontent);
       }
    } else {
...
int cmdbkill(int usern) {
    ...
    if (rc) {
       ...
       log(LOG_INFO,usern,lngtxt(255),user(userp)->nick,irccontent);
...
int cmdlisttasks(int usern)
{
    ...
    pcontext;
    if (user(usern)->parent!=0) userp=user(usern)->parent; else userp=usern;
    log(LOG_INFO,usern,lngtxt(259));
    while(stsk!=NULL)
    {
	if(stsk->uid==usern || user(usern)->rights==RI_ADMIN)
	{
	    log(LOG_INFO,usern,lngtxt(260),stsk->pid,stsk->fdin,stsk->fdout,stsk->fderr,stsk->desc);
	}
	stsk=stsk->next;
    }
    log(LOG_INFO,usern,lngtxt(261));
    return 0x0;
}
...
int cmdname(int usern)
{
	...
	flushconfig();
	log(LOG_INFO,usern,lngtxt(268),me);
	return -1;
    }
}
...
int cmdsetlang(int usern)
{
	...
	log(LOG_INFO,usern,lngtxt(1348),irccontent,user(usern)->nick);
    }
    return -1;
}
...
int checkcrypt(int usern)
{
    ...
    char *prefx;
    char *pt,*pt1,*pt2,*er;
    // mouser 1/14/05, to support more possible prefixes
    //char *b[3];
    char *b[5];
    char inprefix[10];
    int jumpahead=0;
    ...
    char nt[]="\x00";
#ifdef BLOWFISH
    // mouser 1/14/05, we now need a dynamically setable bx prefix
    char bx[20];
#endif
#ifdef IDEA
    char bx[]="[I]";
    ...
    b[1]="[I]";
    b[2]="[N]";
    // mouser 1/15/05, support for more prefixes to decrypt
    b[3]="+OK ";
    b[4]="mcps ";
	...
	*pt2++=0;
	...
	if(strlen(pt)==strlen(buf))
	{
        ...
        // mouser 4/15/05 for testing
		// pt1 is the key for the channel
        //log(LOG_INFO,-1,"using key:");
        //log(LOG_INFO,-1,pt1);
		//
		...
		if(decr==1)
		{
			...
			pt2=NULL;
                        ...
                        // mouser 1/15/05, mouser, original code jumped ahead 3 over the [B], but we only do this now for old blow+idea above,
                        // since other prefixes may have dif lengths or want to skip prefix completely
			jumpahead=0;
			if(strstr(pt,b[0])==pt)
			    {
			    ...
			    jumpahead=3;
                        ...
                        #ifdef COMPILEMIRCRYPTION
			  else if(strstr(pt,b[3])==pt)
			      {
                              // mouser 1/15/05, mircryption support - note we offset +4 since '+OK ' has 4 characters
                              // note that mcps decrypt knows this should be mircryption style encryption because of prefix
				  		if (strlen(pt1)>200)
							pt1[200]=0;
						strcpy(fixedkey,pt1);
						replace(fixedkey,127,'%');
			      pt2=MCPS_stringdecrypt(pt+4,fixedkey,inprefix);
			      prefx=inprefix;
			      }
			  else if(strstr(pt,b[4])==pt)
			      {
                              // mouser 1/15/05, mircryption support - note we offset +5 since 'mcps ' has 5 characters
                              // note that mcps decrypt knows this should be mircryption style encryption because of prefix
				  		if (strlen(pt1)>200)
							pt1[200]=0;
						strcpy(fixedkey,pt1);
						replace(fixedkey,127,'%');
			      pt2=MCPS_stringdecrypt(pt+5,fixedkey,inprefix);
			      prefx=inprefix;
			      }
			#endif
			...
			if(strstr(pt,act)==pt) pt+=strlen(act);
                        ...
                        // mouser 1/15/05, original code jumped ahead 3 over the [B], but we only do this now for old blow+idea above,
                        // since other prefixes may have dif lengths or want to skip prefix completely
			pt+=jumpahead;
		    ...
		    *pt=0;
		    strmncpy(buf,ircbuf,sizeof(buf));
                    ...
                    // mouser, 1/14/05, this doesnt seem to serve any useful purpose so im commenting it out if compiled for mircryption
                    #ifdef COMPILEMIRCRYPTION
                      // not used when mircryption enabled
                    #else
		      if(valid)
...
#ifdef BLOWFISH
                    // mouser 1/15/05, mircryption support - note that by default mircryption encrypt will just call BLOW_ decrypt unless the key starts with 'mcps;'
                    #ifdef COMPILEMIRCRYPTION
                      // IMPORTANT: by default this call to MCPS_stringencrypt will just call BLOW_stringencrypt by default
                      //  and only do mircryption encryption if key has a special prefix.  bx is set by function to indicate proper prefix bx.
				  		if (strlen(pt1)>200)
							pt1[200]=0;
						strcpy(fixedkey,pt1);
						replace(fixedkey,127,'%');
						replace(pt2,127,'%');
				        //log(LOG_INFO,-1,"FIXEDusing key:");
        				//log(LOG_INFO,-1,fixedkey);
                      pt=MCPS_stringencrypt(pt2,fixedkey,bx);
                    #else
                      // normal blowfish encryption,
                      // set bx prefix here
                      strcpy(bx,"[B]");
...
{
    pcontext;
    log(LOG_ERROR,-1,lngtxt(280),user(usern)->login,currentsocket->sock->source);
...
int userinkill(int usern)
{
    char buf[400];
    pcontext;
    log(LOG_WARNING,-1,lngtxt(283),user(usern)->login,currentsocket->sock->source);

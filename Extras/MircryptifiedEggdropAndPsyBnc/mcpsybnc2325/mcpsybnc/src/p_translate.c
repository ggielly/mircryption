...
#ifndef lint
static char rcsid[] = "@(#)$Id: p_translate.c,v 1.1.1.1 2005/02/06 12:56:14 hisi Exp $";
#endif
...
int translatederror(int uid, int err)
{
    ...
    struct translatet *lkm;
    pcontext;
    log(LOG_ERROR,-1,lngtxt(867),uid,err);
    lkm=gettranslate(uid);
    pcontext;
    if(lkm==NULL)
	return 0x0;
...
int cleartranslates()
{
	...
	if(lkm->delayed<=0)
	{
	    log(LOG_WARNING,-1,lngtxt(876),lkm->uid,lkm->dest,lkm->lang);

...
#ifndef lint
static char rcsid[] = "@(#)$Id: menuconf.c,v 1.1.1.1 2005/02/06 12:56:14 hisi Exp $";
#endif
...
struct comopt {
    ...
    char sslpath[200];
    // 1/20/05, mouser, new config.h options
    int compilemircryption;
    int nossl;
    int onlyadminsbwho;
};
...
int readconfigh()
{
	...
	if(config2==NULL) 
	{
        ...
        // 1/20/05, mouser, new confih options
	    comoptions->compilemircryption=false;
	    comoptions->nossl=false;
	    comoptions->onlyadminsbwho=false;
	    return -1;
	}
    }
    comoptions->ctcpversion[0]=0;
	...
	// 1/20/05, mouser, new options
	if(strstr(buf," COMPILEMIRCRYPTION")!=NULL)
	    comoptions->compilemircryption=true;
	if(strstr(buf," NOSSL")!=NULL)
	    comoptions->nossl=true;
	if(strstr(buf," ONLYADMINSBWHO")!=NULL)
	    comoptions->onlyadminsbwho=true;
...
int writeconfig()
{
    ...
    // 1/20/05, mouser, added to support new opions
    fprintf(config,"// 1/16/05, mouser, enable mircryption code compiling\n");
    if(comoptions->compilemircryption)
    	fprintf(config,"#define COMPILEMIRCRYPTION\n\n");
    else
    	fprintf(config,"//#define COMPILEMIRCRYPTION\n\n");

    fprintf(config,"// 1/16/05, mouser, let user disable ssl support if they want\n");
    if(comoptions->nossl)
    	fprintf(config,"#define NOSSL\n\n");
    else
    	fprintf(config,"//#define NOSSL\n\n");
    
    fprintf(config,"// 1/15/05, mouser, i dont think normal users should be able to run bwho (see p_client.c for code)\n");
    if(comoptions->onlyadminsbwho)
   	    fprintf(config,"#define ONLYADMINSBWHO\n\n");
    else
   	    fprintf(config,"//#define ONLYADMINSBWHO\n\n");

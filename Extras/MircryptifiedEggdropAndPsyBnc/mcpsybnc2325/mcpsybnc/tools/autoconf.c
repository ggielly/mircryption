...
#ifndef lint
static char rcsid[] = "@(#)$Id: autoconf.c,v 1.1.1.1 2005/02/06 12:56:20 hisi Exp $";
#endif
...
{
#ifdef NOSSL
  // mouser 1/15/05 - let the user disable ssl from config.h
  return -1;
#endif
...
int main()
{
    ...
    // mouser, 1/15/05, added to locate cygwin openssl exe (some of these are from cygwin patch on web)
	"/bin/openssl.exe",
	"/usr/bin/openssl.exe",
	"/usr/sbin/openssl.exe",
	"/usr/local/bin/openssl.exe",
	"/usr/local/ssl/bin/openssl.exe",
    ...
    // mouser, 1/15/05, for compiling with mixed c++ code, needed for mircryption code stuff
    #ifdef COMPILEMIRCRYPTION
      fprintf(makefile,"LIBS	= -lm -lstdc++ %s %s %s\n",socklib,ssllib,dnslib); /* math lib needed for snprintf of ap */
    #else
    ...
    // mouser 1/15/05 from cygwin patch on web
    #ifdef __CYGWIN__
      fprintf(makefile,"TARGET	= psybnc.exe\n");
    #else
